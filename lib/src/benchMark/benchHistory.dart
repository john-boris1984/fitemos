...
class BenchHistory extends StatefulWidget {
  dynamic historyData;
  BenchHistory({Key key, this.historyData}) : super(key: key);

  @override
  _benchHistoryState createState() => _benchHistoryState();
}
...
class _benchHistoryState extends State<BenchHistory> {
  ...
  _deleteBenchHistory(String itemId, String benchmarkId) async {
    print(itemId);
    // String accessToken = storage.getItem('accessToken')['authentication']['accessToken'];
    String accessToken = SessionManager.getToken();

    api.deleteBenchHistory(accessToken, itemId).then((Account deleteItem) {
      setState(() {
        historyData = deleteItem.returnData['published'];
        isDelete = true;
      });
    }).catchError((error) {
      print(error);
  ...
  void initState() {
    super.initState();
    historyData = itemHistories['published'];
  }
  ...
  showAlertDialog(BuildContext context) {
    ...
    AlertDialog notes = AlertDialog(
      ...
      content: RichText(
          ...
          text: TextSpan(
              text:
                  'Notes Notes Notes Notes Not es Not es Notes No tes Notes Notes No tes Notes Notes Not es Notes Not es Notes Not es Not es Not es Notes Notes Notes No tes Notes ',
              style: GoogleFonts.rubik(
                fontSize: 14,
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: IconButton(
              ...
              onPressed: () {
                _deleteBenchHistory(
                    item['id'].toString(), item['benchmark_id'].toString());
              },
            ),
          ),
        ],
    ...
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          Container(width: width / 3, child: _actionButtons(item)),
        ],
      ),
    );
  }
